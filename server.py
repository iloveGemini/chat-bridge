...
# ====== 代码编辑与检索工具（本地项目操控 / Coding Agent）======
# 单一来源：设定助手白名单与 RP 会话授权都引用这一份，避免两处定义漂移。
CODING_TOOL_NAMES = {
    "list_directory", "read_file_with_lines", "replace_file_lines", "run_terminal_command",
}


def _coding_tool_defs():
    """代码编辑与检索工具 schema（受 _safe_resolve_path 约束在项目根目录内）。"""
    return [
        {"type": "function", "function": {
            "name": "list_directory",
            "description": "列出指定目录下的所有文件和文件夹，用于探索项目结构。",
            "parameters": {"type": "object", "properties": {
                "dirpath": {"type": "string", "description": "相对于项目根目录的路径。比如填 '.' 查看根目录，填 'frontend/js' 查看子目录"}
            }, "required": ["dirpath"]}
        }},
        {"type": "function", "function": {
            "name": "read_file_with_lines",
            "description": "读取本地文件内容。返回的内容会自动在最前面加上行号，请务必根据行号来计算你后续需要修改的代码位置。",
            "parameters": {"type": "object", "properties": {
                "filepath": {"type": "string", "description": "相对于项目根目录的文件路径。例如 'server.py'"}
            }, "required": ["filepath"]}
        }},
        {"type": "function", "function": {
            "name": "replace_file_lines",
            "description": "按行号精确替换文件中的某一段代码。请勿全量写入，只替换你需要改动的行。",
            "parameters": {"type": "object", "properties": {
                "filepath": {"type": "string", "description": "相对于项目根目录的文件路径。"},
                "start_line": {"type": "integer", "description": "要替换的起始行号（包含，从1开始计数）。"},
        ...
        {"type": "function", "function": {
            ...
            "description": "在项目根目录下执行终端命令（如运行测试、跑脚本、检查语法），并获取终端的输出日志。这是你验证代码是否正确的唯一方式。",
            "parameters": {"type": "object", "properties": {
                "command": {"type": "string", "description": "要执行的终端命令，例如 'python -m pytest test.py' 或 'python check.py'"}
            }, "required": ["command"]}
        }},
    ]
...
def _exec_assistant_tool(name, args):
    ...
    try:
        ...
        if name == "list_directory":
            try:
                target_dir = _safe_resolve_path(args.get("dirpath", "."))
        ...
        if name == "read_file_with_lines":
            try:
                target_file = _safe_resolve_path(args.get("filepath"))
                ...
                return {"filepath": args.get("filepath"), "total_lines": len(lines), "content": numbered_content}
            except ValueError as e:
                return {"error": str(e)}
            except Exception as e:
                return {"error": f"读取失败: {str(e)}"}

        if name == "replace_file_lines":
            try:
                target_file = _safe_resolve_path(args.get("filepath"))
                if not target_file.exists() or not target_file.is_file():
                    return {"error": "文件不存在"}
                
                # 转换行号逻辑（人类的第1行在Python列表里是 index 0）
                lines = target_file.read_text(encoding="utf-8").splitlines()
                start_idx = max(1, args.get("start_line", 1)) - 1
                end_idx = min(len(lines), args.get("end_line", len(lines)))
                
                new_lines = args.get("new_content", "").splitlines()
                
                # 切片拼装：保留前面的部分 + 塞入新部分 + 保留后面的部分
                final_lines = lines[:start_idx] + new_lines + lines[end_idx:]
                
                # 写回文件
                target_file.write_text("\n".join(final_lines) + "\n", encoding="utf-8")
                return {"ok": True, "msg": f"成功替换了 {start_idx+1} 到 {end_idx} 行的内容"}
            except ValueError as e:
                return {"error": str(e)}
            except Exception as e:
        ...
        if name == "run_terminal_command":
            ...
            try:
                # cwd=str(ROOT) 确保它只能在项目根目录下运行命令
                # timeout=30 防止它跑了一个死循环脚本把你服务器卡死
                ...
                return {"error": "命令执行超时 (30秒)，已强制终止。"}
            except Exception as e:
                return {"error": f"执行发生异常: {str(e)}"}
        # ------ 👆 新增结束 👆 ------
...
def call_llm_api(session_id):
    ...
    # agentic：按「会话窗口」逐项授权工具，不再吃全局总开关。
    # 主动联系仍受全局 _outreach_enabled() 作为主控兜底；本地项目操控只看本会话授权。
    ...
    if tools_cfg.get("coding"):
        session_tools += _coding_tool_defs()
    if session_tools:
        payload["tools"] = session_tools
        payload["tool_choice"] = "auto"
        log_print(f"🔑 [工具授权][{session_id}] 本会话挂载: "
        ...
        for _round in range(5):
            ...
            for tc in tcs:
                ...
                try:
                    ...
                    a = {}
                fname = fn.get('name', '')
                # 按工具名分派：代码工具走文件/终端执行器，其余走主动联系执行器
                if fname in CODING_TOOL_NAMES:
                    r = _exec_assistant_tool(fname, a)
                else:
                    r = _exec_outreach_tool(fname, a, session_id)
                api_messages.append({"role": "tool", "tool_call_id": tc.get('id'),
                                     "content": json.dumps(r, ensure_ascii=False)})
            log_print(f"🛠️ [角色工具][{session_id}] 执行 {len(tcs)} 个 "
                      f"({', '.join((t.get('function') or {}).get('name','?') for t in tcs)})")
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    NO_SESSION_POST_PATHS = {
        ...
        "/api/worldbooks/create", "/api/worldbooks/update", "/api/worldbooks/delete",
    }
            ...
            if cat in PROMPT_CATEGORIES and name:
                ...
                if cat in ("character", "user"):
                    if data.get("avatar") is not None:
                        ...
                        try:
                            old_av = json.loads(fpath.read_text(encoding="utf-8")).get("avatar")
                            if old_av:
    ...
    def do_GET(self):
        ...
        if path == "/api/prompts/list":
            ...
            if default_user not in user_keys:
                default_user = "default" if "default" in user_keys else (user_list[0]["key"] if user_list else "default")
            # 主身份排最前，其余按 key
