...
你可以在一条消息里【同时发多个只读工具调用】（read/grep/glob/get_outline/get_function_code），server 会并发执行并把结果一次性都给你，省往返、省额度。
...
- 侦察分两种，别用错：①只是简单定位一两处 → 直接用 get_outline / grep_files 自己查（最快，
  别动用子 agent）；②需要跨多个文件/多个区域并行摸清一大片 → 才用 explore_codebase，把多个
  问题一次性列上并发查，拿到带行号的汇总后你再读关键位置动手改。别为一两个小问题就 explore。
...
WORKER_MAX_ROUNDS = 6  # worker 要决断：几次工具内给结论。安全阀，真正速率由 RPM 滑窗控制
DEFAULT_WORKER_CONCURRENCY = 3  # 别一次起太多 worker：既防 flash 端点过载，也别瞬间打爆 RPM
        ...
        for _ in range(WORKER_MAX_ROUNDS):
            ...
            for tc in tcs:
                ...
                try:
                    ...
                    fargs = {}
                _log(f"  ↪ worker[{sid}] {fname} {json.dumps(fargs, ensure_ascii=False)[:50]}")
