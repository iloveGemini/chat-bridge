...
class RateLimiter:
    ...
    def acquire(self, key, rpm):
        ...
        safe_rpm = (rpm - 1) if rpm > 1 else 1
        while True:
            with self._lock:
                now = time.time()
...
- 【侦察先走 explore_codebase】要读懂结构/定位实现/排查多个点时，别自己一个个 read/grep，
  优先用 explore_codebase 把这些问题一次性都列上并发去查，拿到带行号的汇总后，你再自己读那
  几个关键位置并动手改——这样最省你的调用额度、规避 429。
...
WORKER_SYSTEM_PROMPT = (
    "你是一个只读代码研究员。只能用 read_file_with_lines / grep_files / glob_files 读取与检索，"
    "严禁任何写入或执行命令。围绕分配给你的问题去翻代码，最后用简洁要点回答，"
    "并标注来源（文件名:行号）方便主 agent 直接定位。别展开无关内容。"
)
    ...
    return [
        {
            ...
            "function": {
                ...
                "description": (
                    ...
                    "省额度、规避 429。它只读不改；拿到答案后你再自己读那几个关键位置并动手改。"
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
