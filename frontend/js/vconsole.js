...
export class VConsole {
  ...
  initDraggable() {
    ...
    this.fab.addEventListener(
      "touchstart",
      (e) => handleStart(e.touches[0].clientY),
      { passive: true },
    );
    this.fab.addEventListener(
      "touchmove",
      (e) => handleMove(e.touches[0].clientY),
      { passive: true },
    );
    this.fab.addEventListener("mousedown", (e) => handleStart(e.clientY));
    window.addEventListener("mousemove", (e) => {
      if (e.buttons === 1 && startY) handleMove(e.clientY);
    });
  ...
  log(msg, color = "#0f0", ts) {
    ...
    // 【新增】CSS排版优化：保留换行符并允许长单词自动换行
    line.style.whiteSpace = "pre-wrap";
    line.style.wordBreak = "break-all";
    line.style.lineHeight = "1.4";
    line.style.marginBottom = "6px";
    line.style.borderBottom = "1px dashed #333"; // 加一条暗淡的分割线，让大段日志更容易区分
    line.style.paddingBottom = "4px";

    // 【新增】把可能被 JSON 强转义的字面量 "\\n" 还原成真实的换行符 "\n"
