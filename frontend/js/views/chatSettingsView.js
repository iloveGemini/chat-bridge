...
class ChatSettingsView {
  ...
  async open(sessionId) {
    ...
    container.innerHTML =
      '<div style="text-align:center;padding:50px;color:var(--text-secondary);">加载设置中...</div>';
    router.pushView("chat-settings-view");

    try {
      this.sessionData =
        (store.getState().sessions || []).find((s) => s.id === sessionId) || {};
      this.promptsData = await api.fetchPrompts(sessionId);
    } catch (e) {
      container.innerHTML =
        '<div style="text-align:center;padding:50px;color:var(--status-off);">数据加载失败</div>';
      return;
    }
  ...
  bindEvents() {
    ...
    document.getElementById("cs-add-member-btn").onclick = () => {
      ...
      import("./chatMultiSelectView.js").then((m) =>
        m.chatMultiSelectView.open(),
      );
    };

    // 2. 方案选择
    document.getElementById("cs-pick-user").onclick = () => {
      const list = (tree.user || []).map((n) => ({
        name: n,
        label: n,
        selected: n === this.promptsData.active?.user,
      }));
      selectSheet("选择用户身份设定", list, {
        onSelect: async (val) => {
          await api.usePrompt({ user: val }, sid);
          this.open(sid);
        },
      });
    };
    document.getElementById("cs-pick-preset").onclick = () => {
      const list = presets.map((n) => ({
        name: n,
        label: n,
        selected: n === this.promptsData.active?.preset,
      }));
      selectSheet("选择对话执行预设", list, {
        onSelect: async (val) => {
          await api.usePrompt({ preset: val }, sid);
          this.open(sid);
        },
      });
    };
    ...
    if (wbItem) {
      wbItem.onclick = () => {
        import("./chatWorldbooksView.js").then((m) =>
          m.chatWorldbooksView.open(sid),
        );
      };
    }
    ...
    if (bubbleChk) {
      bubbleChk.onchange = (e) => {
        ...
        store.setState({
          config: { ...store.getState().config, bubbleMode: v },
        });
      };
    }
    ...
    if (voiceRule) {
      voiceRule.onclick = () => {
        import("./voiceSettingsView.js").then((m) =>
          m.voiceSettingsView.open(),
        );
      };
    }
    ...
    if (toolsRule) {
      toolsRule.onclick = () => {
        import("./pluginManagerView.js").then((m) =>
          m.pluginManagerView.open(sid),
        );
      };
    }
    ...
    if (searchHistory) {
      searchHistory.onclick = () => {
        import("./chatHistorySearchView.js").then((m) =>
          m.chatHistorySearchView.open(sid),
        );
      };
    }
    ...
    if (longMem) {
      longMem.onclick = () => {
        import("./memoryManagerView.js").then((m) =>
          m.memoryManagerView.open(sid),
        );
      };
    }
